...
def odaberi_slobodni_stol():
    ...
    """
    while slobodno > 0:                         # dok je slobodno > 0 radi
        x = random.randint(0, slobodno-1)       # x = slučajan broj od 0 do slobodno-1
        y = 0                                   # y = 0
        for i in range(BR_STOLOVA):             # za i = 0 do BR_STOLOVA-1 radi
            if stol[i] == 0:                    # ako je stol[i] == 0 tada
                if x == y:                      # ako je x == y tada
                    return i                    # vrati i
                else:                           # inače
                    y += 1                      # y = y + 1
